...
<%
// 요청 URL - product/detail.jsp?no=XXX

// 요청파라미터 조회
int no = Integer.parseInt(request.getParameter("no"));

// 업무로직 수행 - 요청파라미터로 전달받은 상품번호에 해당하는 
Product product = new ProductDao().getProductByNo(no);
// System.out.println(product.getDescription().contains("\r\n"));
%>
<!doctype html>
<html lang="ko">
<head>
		...
		<div class="col-12">
			...
			<table class="table table-bordered ">
				...
				<tbody>
					<tr>
						<th>상품이름</th>
						<td colspan="3"><strong><%=product.getName() %></strong></td>
					</tr>
					<tr>
						<th>상품번호</th>
						<td><%=product.getNo() %></td>
					...
					<tr>
						<th>판매여부</th>
						<td><%="Y".equals(product.getOnSell()) ? "판매중" : "품절" %></td>
						<th>재고수량</th>
